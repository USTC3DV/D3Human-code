...
def shade(
    ...
    if gb_mesh_id is not None:
        buffers['mesh_id'] = torch.cat((gb_mesh_id, alpha), dim=-1)
...
def render_layer(
    ...
    gb_mesh_id, _ = interpolate(mesh.face_labels[None, :, None].float(), rast_out_s, face_normal_indices.int())
...
def render_mesh(
    ...
    def composite_buffer(key, layers, background, antialias):
        ...
        for buffers, rast in reversed(layers):
            ...
            if key == 'mesh_id':
                update_mask = (alpha > 0) & (buffers[key][..., :-1] != 0)  
                accum[update_mask[..., 0]] = torch.cat((buffers[key][..., :-1], torch.ones_like(buffers[key][..., -1:])), dim=-1)[update_mask[..., 0]]
            else:
                accum = torch.lerp(accum, torch.cat((buffers[key][..., :-1], torch.ones_like(buffers[key][..., -1:])), dim=-1), alpha)
            if antialias and key != 'mesh_id':
