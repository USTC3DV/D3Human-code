...
######################################################################################
def write_ply(folder, mesh, save_name=None):
    import os

    if save_name is None:
        ply_file = os.path.join(folder, 'mesh.ply')
    else:
        ply_file = os.path.join(folder, save_name)
    print("Writing mesh: ", ply_file)
    ...
    with open(ply_file, "w") as f:
        ...
        for i in range(num_vertices):
            v = v_pos[i]
            line = "{} {} {}".format(v[0], v[1], v[2])
            if v_nrm is not None:
                n = v_nrm[i]
                line += " {} {} {}".format(n[0], n[1], n[2])
            if v_tex is not None:
                t = v_tex[i]
                line += " {} {}".format(t[0], t[1])
            f.write(line + "\n")
        ...
        # Write face data
        print("    writing {} faces".format(num_faces))
        for i in range(num_faces):
            indices = t_pos_idx[i]
            # Indices in PLY are zero-based
            line = "3 {} {} {}".format(indices[0], indices[1], indices[2])
            f.write(line + "\n")

    print("Done exporting mesh")

def write_obj(folder, mesh, save_name=None, save_material=True):
    if save_name is None:
        obj_file = os.path.join(folder, 'mesh.obj')
    else:
        obj_file = os.path.join(folder, save_name)
